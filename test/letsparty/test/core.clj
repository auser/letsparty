...
(deftest event-system
  (testing "events"
    ...
    (testing "listening to all messages"
      (let [prom1 (promise)
            prom2 (promise)
            prom3 (promise)]
        (listen (fn [msg] (deliver prom1 msg)))
        (listen (fn [msg] (deliver prom2 msg)))
        (listen :akey (fn [msg] (deliver prom3 msg)))
        (publish "Sundays are fundays")
        (publish :akey "Mondays are no fundays")
        (is (= "Sundays are fundays"    (:data @prom1)))
        (is (= "Sundays are fundays"    (:data @prom2)))
        (is (= "Mondays are no fundays" (:data @prom3)))
      )
    )
    (testing "listening once"
      (let [prom1 (promise)
            prom2 (promise)
            x (ref 0)]
        (listen :run (fn [msg]        (dosync (alter x (fn [msg] "listen-always")))))
        (listen-once :run (fn [msg]   (dosync (alter x (fn [msg] "listen-once")))))
        (listen-once :done (fn [msg]  (deliver prom2 msg)))
        (publish :run "dummy msg")
        (publish :run "dummy msg")
        (publish :done "end-this-please")
        (is (= "end-this-please" (:data @prom2)))
        (is (= @x "listen-always"))
      )
    )
  )
